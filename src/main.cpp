...
#if defined(NDEBUG)
# error "Shiacoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Shiacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Shiacoin Signed Message:\n";
...
{
    // Shiacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 420k blocks, which will occur approximately every 2 years
    nSubsidy >>= (nHeight / 420000); 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60;	// Shiacoin: retarget every 1 hour
static const int64 nTargetSpacing = 180;			// Shiacoin: 3 min.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Shiacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    /*
    // Shiacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Shiacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "U.S., Chinese officials meet on cyber security issues: White House - Reuters Sun, 13 Sep 2015 ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 32 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049284710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb7a10f4d4604fa08dce601aaf0f470216fe1b51850b4fbc21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1442113173;
...
unsigned char pchMessageStart[4] = { 0xa2, 0xb2, 0xc2, 0xdb }; // Shiacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FastcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FastcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FastcoinMiner(CWallet *pwallet)
{
    printf("FastcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Shiacoin-miner");
    ...
    try { loop {
        ...
        printf("Running FastcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FastcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FastcoinMiner, pwallet));
}
